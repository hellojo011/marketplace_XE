...
class marketplaceItem extends BaseObject
{
	...
	/**
	 * Get data from database, and set the value to marketplaceItem BaseObject
	 * @param bool $load_extra_vars
	 * @return void
	 */
	function _loadFromDB($load_extra_vars = true)
	{
		...
		// cache controll
		$oCacheHandler = CacheHandler::getInstance('BaseObject');
	...
	function getContentText($strlen = 0)
	{
		...
		$content = preg_replace_callback('/<(BaseObject|param|embed)[^>]*/is', array($this, '_checkAllowScriptAccess'), $content);
		$content = preg_replace_callback('/<BaseObject[^>]*>/is', array($this, '_addAllowScriptAccess'), $content);
	...
	function getComments()
	{
		...
		// Create commentItem BaseObject from a comment list
	...
	function getThumbnailsBaseObject($width = 80, $height = 0, $thumbnail_type = '')
	{
		// Return false if the document doesn't exist
		if(!$this->get('thumbnails_srl')) return;
		// If not specify its height, create a square
