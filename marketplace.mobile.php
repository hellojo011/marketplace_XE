...
class marketplaceMobile extends marketplaceView
{
	...
	function dispMarketplaceMessage($msg_code)
	{
		...
		$oMessageBaseObject = &ModuleHandler::getModuleInstance('message','mobile');
		$oMessageBaseObject->setError(-1);
		$oMessageBaseObject->setMessage($msg);
		$oMessageBaseObject->dispMessage();

		$this->setTemplatePath($oMessageBaseObject->getTemplatePath());
		$this->setTemplateFile($oMessageBaseObject->getTemplateFile());
	}
}
